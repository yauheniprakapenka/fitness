...
Each operation lives under the `FitnessAPI` namespace and within an optional tag: `FitnessAPI(.tagName).operationId`. If an operation doesn't have an operationId one will be generated from the path and method.
...
let getUserRequest = FitnessAPI.User.GetUser.Request(id: 123)
...
Each [Request](#request) also has a `makeRequest` convenience function that uses `FitnessAPI.shared`.
...
There are some options to control how invalid JSON is handled when decoding and these are available as static properties on `FitnessAPI`:
...
- **FitnessAPI.GetExerciseListByUserId**: GET `/api/v1/exercises/`
- **FitnessAPI.GetTrainingsListByTrainerId**: GET `/api/v1/trainings`
